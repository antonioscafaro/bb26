...
\item \textbf{Spring Security}: per autenticazione HTTP Basic, password hashing con BCrypt, autorizzazione basata su ruoli
\end{itemize}
...
\begin{itemize}[leftmargin=1.1em]
\item \textbf{Axios}: per chiamate API REST con interceptors per gestione automatica delle credenziali
\item \textbf{Context API}: per gestione stato globale dell'applicazione
\end{itemize}
...
\item \textbf{Autenticazione}: HTTP Basic tramite Spring Security
