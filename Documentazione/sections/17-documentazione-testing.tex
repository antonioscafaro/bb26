...
\hline
CE-7b &
Stato issue (campo DTO) &
Transizione a RISOLTA (deve impostare \texttt{dataRisoluzione}) &
Transizione a stato diverso da RISOLTA (\texttt{dataRisoluzione} resta null) \\
\hline
\end{longtable}
...
\hline
\texttt{AuthorCannotAssignIssue} &
CE-2 (autore), tentativo non valido di cambiare l'assegnatario (il metodo lancia \texttt{ForbiddenException}) \\
\hline
\texttt{AuthorEditsAllowedFields} &
CE-2 (autore), CE-7 (campi consentiti: titolo, descrizione, priorit\`a) \\
\hline
\texttt{AssigneeCannotEditDescription} &
Ruolo assegnatario, tentativo non valido di modificare la descrizione (il metodo lancia \texttt{ForbiddenException}) \\
\hline
\texttt{AdminSetsLabels} &
CE-1 (amministratore), CE-5 (stesso assegnatario), CE-6 (lista con etichette) \\
\hline
\texttt{AssigneeNotProjectMember} &
CE-1 (amministratore), CE-3 classe non valida (assegnatario non membro del progetto) \\
\hline
\texttt{IntruderUpdates} &
CE-1 classe non valida (utente senza permessi) \\
\hline
\texttt{ResolvedIssueCannotBeModified} &
Issue con stato RISOLTA, tentativo di modifica non consentito (il metodo lancia \texttt{ForbiddenException}) \\
\hline
\texttt{ResolvedIssueCanBeArchived} &
Issue con stato RISOLTA, archiviazione tramite \texttt{archiviaIssueById} (verifica che il metodo dedicato funzioni correttamente) \\
\hline
\texttt{SetsDataRisoluzioneOnResolution} &
CE-1 (amministratore), CE-7b classe valida (transizione a RISOLTA deve impostare \texttt{dataRisoluzione}) \\
\hline
\texttt{DoesNotSetDataRisoluzioneOnOtherStatus} &
CE-1 (amministratore), CE-7b classe non valida (transizione a IN\_LAVORAZIONE non deve impostare \texttt{dataRisoluzione}) \\
\hline
\end{longtable}
...
\begin{itemize}
    \item \textbf{Controllo dei permessi}: il metodo controlla se l'utente \`e amministratore, assegnatario o autore. Abbiamo coperto sia il caso in cui l'utente ha i permessi (amministratore in \texttt{AdminSetsFields}, autore in \texttt{EmptyAssignee} e \texttt{AuthorEditsAllowedFields}) sia il caso in cui non li ha (\texttt{IntruderUpdates} e \texttt{AssigneeCannotEditDescription}, dove ci aspettiamo una \texttt{ForbiddenException}).
    ...
    \begin{itemize}
        ...
        \item email valida, quindi si controlla che l'utente sia membro del progetto. Il caso positivo \`e coperto da \texttt{AdminSetsFields}, mentre quello negativo da \texttt{AssigneeNotProjectMember}. Il test \texttt{AuthorCannotAssignIssue} verifica che un autore non amministratore non possa cambiare l'assegnatario.
    \end{itemize}
    ...
    \item \textbf{Creazione della notifica}: la notifica di assegnazione parte solo quando l'assegnatario cambia. Il test \texttt{AdminSetsFields} verifica che la notifica venga creata, mentre \texttt{AdminSetsLabels} verifica che non venga creata quando l'assegnatario \`e lo stesso di prima.

    \item \textbf{Blocco modifica issue risolta}: il metodo impedisce qualsiasi modifica a una issue con stato RISOLTA. Il test \texttt{ResolvedIssueCannotBeModified} verifica che venga lanciata una \texttt{ForbiddenException}, mentre il test \texttt{ResolvedIssueCanBeArchived} verifica che l'archiviazione avvenga tramite il metodo dedicato \texttt{archiviaIssueById}.

    \item \textbf{Impostazione data di risoluzione}: quando lo stato viene cambiato a RISOLTA, il metodo deve impostare automaticamente il campo \texttt{dataRisoluzione} con il timestamp corrente. Il test \texttt{SetsDataRisoluzioneOnResolution} verifica che il campo venga correttamente popolato, mentre \texttt{DoesNotSetDataRisoluzioneOnOtherStatus} verifica che non venga impostato per transizioni verso stati diversi.
\end{itemize}
